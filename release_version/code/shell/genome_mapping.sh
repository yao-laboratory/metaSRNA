...
echo "mapping top species database number: $number" > $results/genome_mapping_analysis.csv
echo "total sequence number(after clean): $total_number" >> $results/genome_mapping_analysis.csv
echo "$number $total_number" | awk '{print "percentage:", ($1/$2)*100 "%"}' >> $results/genome_mapping_analysis.csv
