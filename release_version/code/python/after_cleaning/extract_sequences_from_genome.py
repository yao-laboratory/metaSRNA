...
def extract_sequences_from_genome(input_blast_result, input_genome, output_fasta):
    ...
    sequence_dict = SeqIO.to_dict(SeqIO.parse(input_genome, "fasta"))
    # print("sequence_dict:",sequence_dict)
    ...
    with open(output_fasta, "w") as output_file:
        for index, row in df_blast_score.iterrows():
            ...
            end_border = len(sequence_dict[sacc])
            if start < end:
                ...
                extend_seq = sequence_dict[sacc].seq[df_start_pos: df_end_pos + 1]
                # print("+")
                # print("extend_whole_seq", sequence_dict[sacc].seq[0:2])
                # print("original start", start)
                # print("original end", end)
                # print("start:",df_start_pos)
                # print("end:",df_end_pos + 1)
                # print("extend_seq", extend_seq)
                SeqIO.write(SeqRecord(extend_seq, id=str(seq_id), description="index_"+str(index)), output_file, "fasta")
                ...
                extend_seq = sequence_dict[sacc].seq[df_start_pos: df_end_pos + 1].reverse_complement()
                # print("-")
                # print("extend_whole_seq", sequence_dict[sacc].seq[0:2])
                # print("original start", start)
                # print("original end", end)
