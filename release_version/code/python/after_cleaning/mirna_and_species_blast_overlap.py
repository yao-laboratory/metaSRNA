...
def find_overlap(folder_path_top_species, folder_path_microRNA, folder_path_overlap):
    # folder_path_microRNA = path.abspath(path.join(path.dirname(__file__), "../../results/blast_hairpinRNA_output/"))
    # folder_path_top_species = path.abspath(path.join(path.dirname(__file__), "../../results/blast_refprok_mapping/"))
    # # folder_path_microRNA = path.abspath(path.join(path.dirname(__file__), "../../results/a/"))
    # # folder_path_top_species = path.abspath(path.join(path.dirname(__file__), "../../results/b/"))
    # folder_path_overlap = path.abspath(path.join(path.dirname(__file__), "../../results/blast_mrna_topspecies_overlap_output/"))
    ...
    txt_files_s =  [file for file in files_s if file.endswith('_filter.txt')]
    ...
    for file in txt_files_combined:
        ...
        if "rna" in file:
            ...
            file_path  = os.path.join(folder_path_top_species, file)
            df = pd.read_csv(file_path, sep=',', header= None)
        # df = []
        # # try:
        # df = pd.read_csv(file_path, sep='\t', header= None)
        ...
        if "rna" in file:
            ...
            total_lines_s.append({'unique_qseqied_column':df_qseq.drop_duplicates().apply(sorted), 'unique_qseqid_number_species':df_qseq['qseqid'].nunique(), 'file_name':path.join(folder_path_top_species, file)})
...
def parse_arguments():
    ...
    parser.add_argument('-species_folder', required=True,
                        type=str, help='species folder', default="none")
...
def main():
    ...
    if args.species_folder and args.hairpin_folder and args.overlap_folder:
        # time_start_s = time.time()
        find_overlap(args.species_folder, args.hairpin_folder, args.overlap_folder)
